...
class PesapalClient
{
    ...
    public function registerIpns()
    {
        return Cache::remember('pesapal_ipn_id', 300, function (): string {
            ...
            ->acceptJson()
            ->retry(100, 5, fn (Exception $exception, PendingRequest $request) => $exception instanceof ConnectionException)
            ->throw()
            ->post(
    ...
    public function getIpnList()
    {
        ...
        ->throw()
        ->get($this->config['base_url'][$this->config['environment']] . '/api/URLSetup/GetIpnList');
    }
