...
class BillableAddress extends Model
{
    ...
    public static function hash(Billable $billable, string $email, string $line1, string $city, string $country): string
    {
        $data = get_class($billable) . "-{$billable->getKey()}-{$email}-{$line1}-{$city}-{$country}";
    ...
    {
        return static::hash($billable, $address->email, $address->line1, $address->city, $address->country);
    }
    ...
    {
        return $this->hasMany(Billing::$billingOrder, 'billing_address_id');
    }

    public function billing_payment_information(): HasMany
    {
        return $this->hasMany(Billing::$billablePaymentProviderInformation, 'billing_address_id');
    }
}